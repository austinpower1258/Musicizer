...
class _HomeState extends State<Home> {
  ...
  Future<List<Post>> _getALlPosts(String text) async {
    ...
    await yt.search.getVideos(text).take(10).forEach((video) => {
          posts.add(new Post(
  ...
  void extractYoutube(String id, Post p) async {
    ...
    var audio = streams.last.url;
     globals.streamPlaylist.add(
          new MusicStream(audio.toString(), p.video));
                                ...
                                onPressed: () {
                                  if (!post.loading) {
                                    if (!post.added) {
                                      ...
                                      globals.streamPlaylist.remove(MusicStream(post.audioStreamUrl, post.video));
